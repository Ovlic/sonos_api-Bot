...
module.exports = {
    ...
    async execute(message, args){
        ...
        if(!args||args.length == 0) return message.channel.send("Provide a room.")
            ...
            groupid = room_check(str_args, _groups)
            if(groupid == null) return message.channel.send("Didnt find Sonos with name "+str_args)
            ...
            if(!sonoswait.error) return message.channel.send("Resumed audio in "+str_args+".")
        }
    }
}